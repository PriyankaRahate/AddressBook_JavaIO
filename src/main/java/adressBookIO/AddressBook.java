...
public class AddressBook {
	...
	static List<ContactList> addressBook;
	private static Object deleteContactList;

	public static void main(String[] args) {
		addressBook = new LinkedList<ContactList>();

		boolean isExit = false;

		System.out.println("Welcome to the Address book,  your contactsList " );
		Scanner scanner = new Scanner(System.in);
		while (!isExit) {
			System.out.println("Select the option from below");
			if (addressBook.isEmpty()) {
				System.out.println("1. Add ContactList" + "\n5. Exit");
			} else {
				System.out.println(
						"1. Add ContactList" + "\n2. Display ContactList\n3. Edit ContactList" + "\n4. Delete ContactList\n5. Exit");
			}
	...
	private static void addContact(Scanner scanner) {
		...
		System.out.println("ContactList has been saved.");
	}
	...
	private static void editContactDetails(ContactList editContactList, Scanner scanner) {
		System.out.println("Enter First Name: ");
		String firstName = scanner.nextLine();
		editContactList.setFirstName(validateFirstName(firstName, scanner));

		System.out.println("Enter Your Email: ");
		String email = scanner.nextLine();
		editContactList.setEmail(validateEmail(email, scanner));

		System.out.println("Enter Phone Number: ");
		String phoneNumber = scanner.nextLine();
		editContactList.setPhoneNumber(validatePhone(phoneNumber, scanner));
	...
	private static void deleteContact(Scanner scanner) {
		...
		ContactList deleteContact = null;
		for (int i = 0; i < addressBook.size(); i++) {
			if (firstName.equals(addressBook.get(i).getFirstName())) {
	...
	private static void showContacts() {
		if (addressBook.isEmpty()) {
			...
			for (ContactList contact : addressBook) {
